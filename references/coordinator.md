...
candidate; any path outside the named task turns the observed state into `blocked`, so the live
table cannot advertise an unsafe candidate as ready. Missing objects, unreadable ancestry, and
incomparable bases are blocking. `evidence_url`, when present, must be a reachable HTTP(S) or
Agentcloud artifact URL; a path on the worker host is not a handoff link.
...
the carried candidate task tree is byte-identical too and rechecks every commit in both stacks for
scope. Only then are explicitly whitelisted tree-invariant checks carried (`oracle`,
